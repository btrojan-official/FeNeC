...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert CSV hyperparameter files to JSON configs.")
...
def main():
    ...
    for filename in os.listdir(args.input_dir):
        if filename.endswith(".csv"):
            ...
            df['average_of_average_accuracy'] = df['average_of_average_accuracy'].apply(lambda x: float(str(x).replace(',', '.')))
