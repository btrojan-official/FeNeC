...
def plot_method_scores(df: pd.DataFrame, title: str) -> None:
    ...
    for i, method in enumerate(methods_names):
        ...
        plt.plot(method_data["task"], method_data["mean_accuracy"], marker=markers[i], linestyle=linestyles[i], label=method, linewidth=2, markersize=8, alpha=alpha) # , color=colors[i]
